...
export default function RecipeDetails() {
  const history = useHistory(); const { id } = useParams();
  const location = useLocation(); const [recipe, setRecipe] = useState();
  ...
  const fetchingData = useCallback(async () => {
  ...
  }, [id, type]); const verifyDoneLocalStorage = useCallback(() => {
    const doneRecipe = localStorage.getItem('doneRecipes')
      ? JSON.parse(localStorage.getItem('doneRecipes')) : [];
    if (doneRecipe) {
      const isDone = doneRecipe.some((done) => Number(done.id) === Number(id));
      if (isDone) setRecipeDone(true);
    }
  }, [id]); const verifyFavoriteLocalStorage = useCallback(() => {
    ...
    setContinueRecipe(true); history.push(`/${type}/${id}/in-progress`);
  };
  const handleShare = () => {
    const url = window.location.href; setShowCopyMsg(true);
    window.navigator.clipboard.writeText(url);
  };
  const addFavoriteRecipe = () => {
    setFavRecipe(); verifyFavoriteLocalStorage();
  };
  const getInProgressRecipes = useCallback(() => {
    ...
    if (inProgressRecipes) {
      const isInProgress = Object.keys(
        inProgressRecipes[mealsOrCocktails],
      ).some((inProgressRecipe) => inProgressRecipe === id);
      if (isInProgress) setContinueRecipe(true);
    }
  }, [id, verifyRoute]);
  useEffect(() => {
    fetchingData(); getInProgressRecipes();
    verifyDoneLocalStorage(); verifyFavoriteLocalStorage();
  }, [fetchingData, getInProgressRecipes, id, verifyDoneLocalStorage,
    verifyFavoriteLocalStorage, verifyRoute]);
  useEffect(() => {
    if (recipe === undefined || recomendations === undefined) return;
    const maxIngredient = 20; const newArrI = []; const newArrQ = [];
    for (let index = 1; index <= maxIngredient; index += 1) {
      const keyI = `strIngredient${index}`; const keyQ = `strMeasure${index}`;
      if (type === 'foods') {
        newArrI.push(recipe.meals[0][keyI]); newArrQ.push(recipe.meals[0][keyQ]);
      } else {
        newArrI.push(recipe.drinks[0][keyI]); newArrQ.push(recipe.drinks[0][keyQ]);
      }
    }
    setIngredients([...newArrI].filter(Boolean));
    setIngredientsQntd([...newArrQ].filter(Boolean)); setLoading(false);
    ...
    <div>
      ...
      {!loading && (
        <div className="">
          ...
          <Carousel
            data={
              type === 'foods' ? recomendations.drinks : recomendations.meals
            }
            type={ type }
          />
        </div>
